...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("TESTACTIN: found %d as leader1", leader1)
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	DPrintf("TESTACTION: %d (leader1) disconnects", leader1)
	cfg.checkOneLeader()
	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	DPrintf("TESTACTION: %d (leader1) rejoins", leader1)

	leader2 := cfg.checkOneLeader()
	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	DPrintf("TESTACTION: %d (leader2) disconnects", leader2)
	...
	cfg.connect(leader2)
	DPrintf("TESTACTION: %d (leader2) rejoins", leader1)
	cfg.checkOneLeader()
	cfg.end()
}
